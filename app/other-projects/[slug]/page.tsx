...
import { OTHER_PROJECTS_ITEMS_QUERY } from "@/sanity/lib/queries";

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  const resolvedParams = await params;
  const project = (await sanityFetch({ query: OTHER_PROJECTS_ITEMS_QUERY, params: resolvedParams })).data as SanityDocument | null;
...
export default async function OtherProjectDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  const resolvedParams = await params;
  const project = (await sanityFetch({ query: OTHER_PROJECTS_ITEMS_QUERY, params: resolvedParams })).data as SanityDocument | null;
      ...
      <section className="py-20 bg-gradient-to-b from-background to-secondary/20">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
            ...
            <Link href="/other-projects" className="btn-outline hover:bg-primary hover:text-primary-foreground">
              View More Work
            </Link>
          </div>
